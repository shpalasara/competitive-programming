...
class varun_Sep_17 {
    ...
    static long modi = 1000000007L;
    static void solve1(){
        int t = i();
        while(t-->0){
            int n = i();
            int m = i();
            int temp[][] = new int[m+1][4];
            long update[] = new long[n+1];
            long query[] = new long[m+1];
            for(int i=1;i<=m;i++){
                temp[i][1]=i();
                temp[i][2]=i();
                temp[i][3]=i();
            ...
            query[m]=0;
            for(int i=m;i>0;i--){
                operation = (operation + query[i])%modi;
                
                if(temp[i][1] == 1){
                    update[temp[i][2]]=(update[temp[i][2]]+operation)%modi;
                    if(temp[i][3]+1<=n){
                        update[temp[i][3]+1]=(update[temp[i][3]+1] - operation + modi)%modi;
                    }
                }else{
                    query[temp[i][3]] = (query[temp[i][3]] + operation)%modi;
                    query[temp[i][2]-1] = (query[temp[i][2]-1] - operation + modi)%modi;
                }
            }
            for(int i=2;i<=n;i++){
                update[i] = (update[i] + update[i-1])%modi;
            }
            for(int i=1;i<=n;i++){
